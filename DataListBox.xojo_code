...
Inherits DesktopListBox
	...
	#tag Method, Flags = &h0
		Sub SetEqualRelativeWidths()
		  dim widths() as String
		  dim widthsParameter as string
		  dim percent as Integer = 100 / ColumnCount
		  
		  for i as Integer = 0 to LastColumnIndex
		    Widths.Add percent.ToString + "%"
		  next i
		  
		  widthsParameter = String.FromArray(widths , ",")
		  ColumnWidths = widthsParameter
		  
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Function ShowData(db as SQLiteDatabase, Query as String) As string
		  if Query.Trim = "" then
		    ResetListBox
		    Return ""
		  end if
		  ...
		  try
		    ...
		    Redim listrow(rows.LastColumnIndex)
